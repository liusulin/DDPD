...
@dataclass
class GPTConfig:
    model_type: str = 'ddpd_denoiser_mask' # 'ddpd_denoiser_mask' or 'ddpd_denoiser_uniform'
    block_size: int = 256
    vocab_size: int = 28 
    n_layer: int = 12
    n_head: int = 12
...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        """
        config.vocab_size should include a mask token for flow and d3pm, but not for dch
        """
        super().__init__()
    ...
    def forward(self, idx, time, targets=None, target_mask=None, do_self_conf_loop=False, attn_mask=None, time_int=None):
        """
        ...
        masked_loss = loss * target_mask.view(-1) # only calculate loss on noisy positions

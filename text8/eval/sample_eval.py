...
parser.add_argument("--path", help="path to folder that contains the sample folders")
...
sample_folders = glob(os.path.join(folder_path, 'samples_*'))
nll_list = []
entropy_list = []
for sample_folder in sample_folders:
    ...
    if os.path.exists(os.path.join(sample_folder, 'status.txt')):
        ...
        # read nll and entropy from the folder
        nll = np.loadtxt(os.path.join(sample_folder, 'loss_mean.txt'))
        entropy = np.loadtxt(os.path.join(sample_folder, 'entropy.txt'))
        nll_list.append(nll)
    ...
    nll_list.append(np.mean(losses))
    ...
    np.savetxt(os.path.join(sample_folder, 'entropy.txt'), np.array([sample_entropy]))

    # write done in status.txt
    with open(os.path.join(sample_folder, 'status.txt'), 'w') as f:
        f.write('done')

# save nll_list and entropy_list in the out_folder
np.savetxt(os.path.join(folder_path, 'nll_list.txt'), np.array(nll_list))
np.savetxt(os.path.join(folder_path, 'entropy_list.txt'), np.array(entropy_list))

...
def get_dataloaders(config, distributed=True):
    ...
    train_set = get_dataset(config.data.train, "train", cache_dir=config.data.cache_dir, block_size=config.model.length, num_proc=config.data.num_proc)
    valid_set = get_dataset(config.data.valid, "validation" if (config.data.valid != "lm1b" and config.data.valid != "text8") else "test", cache_dir=config.data.cache_dir, block_size=config.model.length, num_proc=config.data.num_proc)

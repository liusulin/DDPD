...
class Uniform(Graph):
    ...
    def sample_transition_and_index(self, i, sigma):
        move_chance = 1 - (-sigma).exp()
        move_indices = torch.rand(*i.shape, device=i.device) < move_chance
        i_pert = torch.where(move_indices, torch.randint_like(i, self.dim), i)
        return i_pert, move_indices, move_chance
    ...
    def binary_if_noise_entropy(self, logits, is_noise, weighted_by_time=False):
        loss_is_noise = F.binary_cross_entropy_with_logits(logits.squeeze(-1), is_noise.float(), reduction="none")
        if weighted_by_time:
            loss_is_noise = loss_is_noise.sum(dim=-1) / is_noise.sum(dim=-1).clamp(min=1.0)
        else:
            loss_is_noise = loss_is_noise.mean(dim=-1)
        return loss_is_noise.mean()

...
class GeometricNoise(Noise, nn.Module):
    """
    Geometric noise schedule built so that sigma_min^(1-t) * sigma_max^t interpolates between sigma_min and sigma_max   
    when t goes from 0 to 1. Used for absorbing
    
    Total noise is sigma_min^(1-t) * sigma_max^t / log(sigma_max / sigma_min)
    """

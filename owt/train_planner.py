...
def main(cfg):
    ...
    if cfg.load_dir is not None:
        ...
        print(cfg)
        cfg.work_dir = load_dir
        cfg.wandb.id = wandb_id
        cfg.wandb.is_resume = is_resume
    else:
    ...
    logger = utils.get_logger(os.path.join(cfg.work_dir, "logs"))
    ...
    try:
        ...
        mp.spawn(run_train_planner.run_multiprocess, args=(ngpus, cfg, port), nprocs=ngpus, join=True)

...
wandb_project = 'text8'
wandb_run_name = 'ddpd_denoiser' # 'run' + str(time.time())
...
assert model_type in ['ddpd_denoiser_mask', 'ddpd_denoiser_uniform']
...
def corrupt_data(data, times):
    ...
    if model_type == 'ddpd_denoiser_mask':
        u = torch.rand((b, t), device=times.device)
        target_mask = u < (1.0 - times.view(b, 1)) # using linear noise schedule
        data[target_mask] = mask_token_id
    elif model_type == 'ddpd_denoiser_uniform':
        u = torch.rand((b, t), device=times.device)
        target_mask = u < (1.0 - times.view(b, 1)) # using linear noise schedule
        data[target_mask] = torch.randint(0, meta_vocab_size-1, (b, t), device=times.device)[target_mask]
    else:
...
def calc_loss(X, Y, times, target_mask, infill_probs, num_ones_in_mask):
    loss = model(X, times, Y, target_mask)
    return loss

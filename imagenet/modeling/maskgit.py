"""This file contains implementation for MaskGIT model for denoiser and planner in DDPD.
...
class ImageBert(nn.Module, PyTorchModelHubMixin):
    ...
    def generate(self,
                 ...
                 conf_anneal=True,
                 conf_method="logit", # "logit" - MaskGit or "random" - discrete diffusion sampling
        ...
        for step in range(num_sample_steps):
            ...
            logit_annealed_temp = logit_temperature * (1.0 - ratio) if logit_temp_anneal else logit_temperature
            ...
            mask_len = torch.maximum(torch.Tensor([1]).to(device),
                                     torch.minimum(torch.sum(is_mask, dim=-1, keepdims=True) - 1,
                                                   mask_len))[0].squeeze()
            if conf_method == "logit":
                sampled_logits = torch.squeeze(
                    torch.gather(logits, dim=-1, index=torch.unsqueeze(sampled_ids, -1)), -1)
                confidence = torch.where(is_mask, sampled_logits, +np.inf).float()
                confidence = add_gumbel_noise(confidence, annealed_temp)
            elif conf_method == "random":
                confidence = torch.rand_like(ids, dtype=torch.float)
                confidence = torch.where(is_mask, confidence, +np.inf)
            else:
...
class ViTPlanner(nn.Module, PyTorchModelHubMixin):
    def __init__(self, config):
        ...
        self.model.lm_head = nn.Linear(self.hidden_size, 1, bias=True)
        self.model.post_init()
    ...
    def forward(self, input_ids=None, condition=None, cond_drop_prob=0.1):
        ...
        model_output = self.model(input_ids=input_ids)
        model_output = model_output[0]
        return self.model.lm_head(model_output[:, 1:]).squeeze(-1)  # remove cond, squeeze if single logit


class UViTPlanner(ViTPlanner):
    def __init__(self, config):
        super().__init__(config=config)
        ...
        self.norm = nn.LayerNorm(self.hidden_size)
        self.lm_head = nn.Linear(self.hidden_size, 1, bias=True)
        self.apply(self._init_weights)
    ...
    def forward(self, input_ids=None, condition=None, cond_drop_prob=0.1):
        ...
        for blk in self.out_blocks:
            x = blk(x, skips.pop())
        x = self.norm(x)
        return self.lm_head(x[:, 1:]).squeeze(-1) # remove cond,  squeeze if single logit